...
### White Wines
```{r echo = FALSE}
# Return the given data frame without data that fall outside 2.5 times the IQR
# from the mean (+/-) for the given feature. It returns the filtered data frame.
filter_outliers_by_feature <- function(df, feature) {
  series <- df[, feature]
  iqr    <- IQR(series)
  mean   <- mean(series)
  lower_fence   <- mean - 2.5 * iqr
  upper_fence   <- mean + 2.5 * iqr
  filtered_data <- df[series > lower_fence & series < upper_fence, ]
  return(filtered_data)
}

print_plot_summary <- function(x_series,
                               x_label,
                               num_x_outliers,
                               y_series,
                               y_label,
                               num_y_outliers) {
  print(paste('Outliers have been filtered out that lie outside of 2.5 times',
              'the IQR from the mean (+/-).'))
  x_outlier_msg  <- paste('Number of data considered outliers for ',
                          x_label,
                          ': ',
                          num_x_outliers,
                          sep = '')     
  y_outlier_msg  <- paste('Additional data considered outliers for ',
                          y_label,
                          ': ',
                          num_y_outliers,
                          sep = '')  
  ...
  print('---------------------------------------------------------------------')
  print(paste('Summary statistics for', x_label, '(excluding outliers)'))
  print(describe(x_series))

  print('---------------------------------------------------------------------')
  print(paste('Summary statistics for', y_label, '(excluding outliers)'))
  print(describe(y_series))
}
...
scatter_plot <- function(data, data_label, x_var, x_label, y_var, y_label) {
  ...
  filtered_data  <- filter_outliers_by_feature(data, x_variable)
  num_x_outliers <- nrow(data) - nrow(filtered_data)
  data           <- filtered_data

  filtered_data  <- filter_outliers_by_feature(data, y_variable)
  num_y_outliers <- nrow(data) - nrow(filtered_data)
  data           <- filtered_data

  p <-  ggplot(data, aes_string(x = x_variable, y = y_variable)) +
          ggtitle(paste(data_label, ': ', x_label, ' vs ', y_label, sep = '')) +
          geom_point(shape = plot_point_shape, alpha = plot_point_alpha) +
          geom_smooth()
  print(p)

  x_series <- data[, x_variable]
  y_series <- data[, y_variable]
  print_plot_summary(x_series,
                     x_label,
                     num_x_outliers,
                     y_series,
                     y_label,
...
alcohol_scatter_plot_by_quality <- function(data,
                                            ...
                                            y_label,
                                            colour_palette) {
  x_variable <- deparse(substitute(alcohol))
  y_variable <- deparse(substitute(y_var))
  ...
  p <-  ggplot(data, aes_string(y = deparse(substitute(y_var)))) +
          ggtitle(title) +
          ...
          theme(panel.background  = element_rect(fill = light_grey),
                legend.key        = element_rect(fill = light_grey),
...
```{r echo = FALSE, fig.width = 12}
ggplot(whites, aes(x = alcohol, y = density, colour = factor(quality))) +
  ggtitle("White Wines: Alcohol vs Density (Faceted by Quality)") +
  geom_point(shape = plot_point_shape) +
  scale_colour_brewer(palette = white_wines_palette) +
  theme(panel.background  = element_rect(fill = light_grey),
        legend.key        = element_rect(fill = light_grey),
        legend.background = element_rect(fill = light_grey)) +
  facet_grid(. ~ quality)
...
165 outliers is quite a few. Let's take a look to see what the plot looks like when we include these outliers.

```{r echo = FALSE, fig.width = 12}
ggplot(whites, aes(y = chlorides)) +
...
ggplot(reds, aes(x = alcohol, y = volatile.acidity, colour = factor(quality))) +
  ...
  geom_point(shape = plot_point_shape) +
  scale_colour_brewer(palette = red_wines_palette) +
  theme(panel.background  = element_rect(fill = light_grey),
        legend.key        = element_rect(fill = light_grey),
        legend.background = element_rect(fill = light_grey)) +
  facet_grid(. ~ quality)
...
final_plot_with_theme <- function(p) {
  marg <- margin(t = 10, b = 10, r = 10, l = 10)
  p <-  p  +
          scale_colour_manual(values = wine_colours_against_black_lines,
                              name   = "Wine Colour") +
          scale_fill_manual(values = wine_colours_against_black_fills,
                            name   = "Wine Colour") +
          theme(title            = element_text(size = 30, margin = marg),
                axis.text.x      = element_text(size = 20, margin = marg),
                axis.text.y      = element_text(size = 20, margin = marg),
                axis.title.x     = element_text(size = 28, margin = marg),
                axis.title.y     = element_text(size = 28, margin = marg),
                legend.title     = element_text(size = 30, margin = marg),
                legend.text      = element_text(size = 28, margin = marg),
...
x_variable <- deparse(substitute(density))
y_variable <- deparse(substitute(volatile.acidity))

filtered_data  <- filter_outliers_by_feature(whites_and_reds, x_variable)
num_x_outliers <- nrow(whites_and_reds) - nrow(filtered_data)
data           <- filtered_data
...
p <-  ggplot(data,
             aes(x     = density,
                 ...
                 color = wine.colour,
                 size  = alcohol)) +
        ggtitle("Red and White Wines: Density vs Acidity (Faceted by Quality)") +
        geom_point(shape = plot_point_shape) +
        scale_x_continuous(breaks = c(0.99, 1.0)) +
        labs(x = 'Wine Density (g / cm^3)', y = 'Volatile Acidity (g / dm^3)') +
        scale_size(name = 'Alcohol\n(% by volume)') +
...
# print(paste('Num outliers for density:', num_x_outliers))
# print(paste('Num outliers for volatile.acidity:', num_y_outliers))
```
**NB** This plot excludes outliers that fall outside 2.5 times the interquartile range from the mean (+/-). This has excluded 2 outliers for wine density and 153 outliers for volatile acidity. The plots are clearer without these outliers and the general trend of the plot is unaffected.
